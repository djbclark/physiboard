...
import brobata.physiboard.commands.CommandJson
...
import brobata.physiboard.inputmethod.ui.KeyboardThemeColors
import brobata.physiboard.inputmethod.expansion.ExpansionActivationPolicy
import brobata.physiboard.inputmethod.expansion.ExpansionPresentation
import brobata.physiboard.inputmethod.expansion.TextExpansionEngine
...
object SettingsManager {
    ...
    fun resolveEffectiveSoftwareKeyboardMode(context: Context): SoftwareKeyboardMode {
        ...
        return brobata.physiboard.inputmethod.SoftwareKeyboardAutoDetector.resolve(context)
    }
    ...
    fun isRawModeApp(context: Context, packageName: String?): Boolean {
        ...
        return packageName in brobata.physiboard.inputmethod.WebApkHost.expandWithHosts(context, packages)
    }
    ...
    // ---------------------------------------------------------------------------------------
    // Notification ring: a glow around the camera hole while the screen is otherwise off.
    // ---------------------------------------------------------------------------------------

    fun isNotificationRingEnabled(context: Context): Boolean =
        getPreferences(context).getBoolean(KEY_NOTIFICATION_RING_ENABLED, false)

    fun setNotificationRingEnabled(context: Context, enabled: Boolean) {
        getPreferences(context).edit().putBoolean(KEY_NOTIFICATION_RING_ENABLED, enabled).apply()
    }

    /** Whether the waiting apps' icons are drawn below the ring. Off: just the ring. */
    fun isNotificationRingIconsEnabled(context: Context): Boolean =
        getPreferences(context).getBoolean(KEY_NOTIFICATION_RING_ICONS, false)
    ...
    fun getNotificationRingMinutes(context: Context): Int =
        getPreferences(context).getInt(KEY_NOTIFICATION_RING_MINUTES, NOTIFICATION_RING_DEFAULT_MINUTES)

    fun setNotificationRingMinutes(context: Context, minutes: Int) {
        getPreferences(context).edit().putInt(KEY_NOTIFICATION_RING_MINUTES, minutes).apply()
    }

    fun getNotificationRingBrightness(context: Context): brobata.physiboard.ring.RingBrightness =
        brobata.physiboard.ring.RingBrightness.fromName(
            getPreferences(context).getString(KEY_NOTIFICATION_RING_BRIGHTNESS, null)
        )

    fun setNotificationRingBrightness(context: Context, brightness: brobata.physiboard.ring.RingBrightness) {
        getPreferences(context).edit().putString(KEY_NOTIFICATION_RING_BRIGHTNESS, brightness.name).apply()
    }

    /**
     * A hand-fitted ring, in window pixels, or null to use the fitted default. The default is
     * measured off one phone; panels differ by a few pixels, so users can refit it.
     */
    fun getNotificationRingOverride(context: Context): brobata.physiboard.ring.RingGeometry.Ring? {
        val prefs = getPreferences(context)
        if (!prefs.contains(KEY_NOTIFICATION_RING_RADIUS)) return null
        return brobata.physiboard.ring.RingGeometry.Ring(
    ...
    fun setNotificationRingOverride(context: Context, ring: brobata.physiboard.ring.RingGeometry.Ring) {
        getPreferences(context).edit()
    ...
    fun getBounceKeysCategoryEnabled(
        context: Context,
        category: brobata.physiboard.inputmethod.BounceKeyFilter.Category
    ): Boolean {
        return when (category) {
            brobata.physiboard.inputmethod.BounceKeyFilter.Category.CHARACTER ->
                getBounceKeysCharacterKeysEnabled(context)
            brobata.physiboard.inputmethod.BounceKeyFilter.Category.MODIFIER ->
                getBounceKeysModifierKeysEnabled(context)
            brobata.physiboard.inputmethod.BounceKeyFilter.Category.SPACE ->
                getBounceKeysSpaceEnabled(context)
            brobata.physiboard.inputmethod.BounceKeyFilter.Category.ENTER ->
                getBounceKeysEnterEnabled(context)
            brobata.physiboard.inputmethod.BounceKeyFilter.Category.BACKSPACE ->
                getBounceKeysBackspaceEnabled(context)
            brobata.physiboard.inputmethod.BounceKeyFilter.Category.UNSUPPORTED -> false
        }
    }
    ...
    private fun isLayoutAvailable(context: Context, layoutName: String): Boolean {
        if (brobata.physiboard.data.layout.LayoutFileStore.layoutExists(context, layoutName)) {
            return true
        }
        return try {
